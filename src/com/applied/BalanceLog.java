...
public class BalanceLog {
	public double balancePoint(Log log) {
		double len = log.length();
		double total = log.weightUpto(len);
		
		double left = 0.0;
		for (int i = 0; i < len; i++) {
			total = total - log.weightUpto(i);
			if(Double.compare(left, total) == 0){
				return log.weightUpto(i);
			}
			
			left = Double.sum(left, log.weightUpto(i));
		}
